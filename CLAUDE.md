...
The website of **Both Clocks**, the owner's endurance brand. The model: the owner takes on hard endurance events by choice; each event sponsors one charity serving people in unchosen suffering (cancer, hunger, abuse, neglect, disease, loss — keep this broad, never one cause); donations go directly to the sponsored charity, and merch profits are donated too (owner keeps break-even only, and is explicit about not profiting). Both Clocks also sponsors **teams**: the long-term model is groups and individuals running under the banner across the USA and world, and recruiting is one of the site's asks. First event: a 100-mile ultramarathon in 24 hours (Southern Tour Ultra, Wilmington, NC, Jan 15–16, 2027), where the owner runs the 100 individual and Both Clocks aims to sponsor a relay team. **First sponsored cause: the Wilmington Health Foundation**, raised through a fundraising page created with the race. `FUNDRAISE_URL` is the placeholder in every donate button — replace all occurrences when the owner supplies the real fundraising page link (donate links must always point at the charity's/campaign's own page, never a personal account).
...
- **Placeholder content**: Elements marked `class="edit"` (dashed underline) and `<!-- EDIT -->` comments are draft content awaiting the site owner's real values — currently the `FUNDRAISE_URL` donate buttons, the shop's two product cards, the relay-recruiting mailto, and the contact/Instagram/LinkedIn links in the footer. The Why page is finished copy in the owner's voice (written from an interview) — don't rewrite it without being asked. Preserve these markers when editing unless the user is supplying the real content.
