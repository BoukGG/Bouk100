...
- **One-page scroll**: Six sections (`#home`, `#why`, `#causes`, `#race`, `#log`, `#shop`) form one continuous scroll narrative — there is no view switching. `[data-go]` elements smooth-scroll to their section; a fixed nav scrollspies the sections (IntersectionObserver) and highlights the active tab (pill style); a 2px bone scroll-progress line runs along the top. The Shop is reached via the nav CTA button, not a tab.
- **Chapter rails**: each band's content sits in a `.shell .chap` grid with a sticky numbered rail on the left (`.rail`, ghost numeral + mono label) that folds inline under 1000px. New sections should follow the same pattern and keep the numbering sequential.
- **Hero animation**: the mark performs the brand story on load — the heavy ring draws itself around the dial (stroke-dasharray) and stops, the finish tape drops into the noon gap, then the unbroken inner ring fades in and breathes forever (slow opacity pulse). Reduced motion renders the finished mark, no breathing. A slow marquee strip (`.marq`) runs under the hero.
- **Night-to-dawn arc**: the page background deliberately runs dark → light down the page (carbon hero/clocks/why → slate causes band → `.dawn` grayscale gradient divider with a first-light glow → bone paper for race/log/shop → carbon footer), mirroring the race's night in monochrome. Preserve this order when adding sections.
