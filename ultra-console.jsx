...
/* All plan data, the completed-session record, and public daily actuals
   live in training-data.json — the single source of truth shared with
   index.html's live-training block. console.html fetches it and sets
   window.TRAINING_DATA before compiling this file. */
const DATA = window.TRAINING_DATA;
const RACE_DATE = new Date(DATA.raceDate);
...
export default function UltraConsole() {
  ...
  const [logs, setLogs] = useState(ACTUALS);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (r?.value) setLogs({ ...ACTUALS, ...JSON.parse(r.value) });
      } catch (e) { /* first run */ }
      setLoading(false);
    })();
  }, []);
